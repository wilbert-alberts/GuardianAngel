...
void BensSim::setupGSM (const char *pincode)
{
    ...
    {
        pSIM->pinCode (SET, pincode); // WARNING! Be certain that you input the correct pin code!
...
// returns T: success
{
    ...
    pos = strstr (pos, ",");
    if (pos) {   // do crap and time of submit
       _dooa = prev;
       *pos++ = '\0';
       _sentAt = pos;
    }    
...
void BensSim::extractAllMessages (char* buffer) 
{
   ...
   while (line != NULL) {
       if (strstr (line, newSms) == line) { // it is an SMS intro line
           if (inProgress) {
       ...
       else if (strstr (line, endSms) == line) { // all SMS'es done
           success = true;
           break;
       }
       ...
       else { // it is the first or next line of an SMS message
...
void BensSim::getAllSMSes ()
{
   ...
   if (debug) SerialMon.println ("Handle unsolicited messages");   
   handleUnsolicited();

   if (debug) SerialMon.println ("Getting list of all SMS'es (smsList)...");  
   pSIM->formatReply (false);
   pSIM->smsList (SET, "\"ALL\"");
   reportIfError ("smsList");

   if (isOK()) {
      if (debug) SerialMon.println ("Extract messages");
      extractAllMessages (pSIM->getBuffer ());
      if (debug) SerialMon.println ("extract done");
   }
}