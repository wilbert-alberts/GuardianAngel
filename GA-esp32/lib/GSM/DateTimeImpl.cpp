...
// split a string into date/time components
{
   ...
   if (!success) {
      ...
      nMatches = sscanf (theString, "%d/%d/%d,%d:%d:%d%d",
                                     &year, &month, &day, &hour, &min, &sec, &tz);
      if ((nMatches == 7) || (nMatches == 6)) { // with or without timezone
         success = true;
      }
   }
   if (!success) {
      ...
      if (nMatches >= 3) { // we have at least the date
         ...
         if (month > 0) success = true;
      }
   }

   if (!success) {
      // no date, check for time
      year = 0; month = 1; day = 1; hour = 0; min = 0; sec = 0; tz = 0;
      //                              12:21:15+4
...
DateTime::DateTime (int yy, int mm, int dd, int h, int m, int s, int dst)
{
   ...
   for (int i = 1; i < mm; i++) days += daysInMonth (i, yy);
   days += dd-1;
   seconds = (h * 60 + m) * 60 + s + dstToSeconds (dst);
   normalize ();
}
...
{
   if (dstValue < 0 || dstValue > 2) dstValue = 0;
...
int DateTime::compareTo (const DateTime &other) {
	...
	if (days  < other.getDays     ())      return -1;
	if (days  > other.getDays     ())      return 1;
...
int DateTime::compareTimeTo (const DateTime &other) {
	...
	if (seconds   < other.getSeconds ())   	return -1;
	if (seconds   > other.getSeconds ())		return 1;
